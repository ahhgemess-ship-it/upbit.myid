...
export default function FlashSaleCard({ product, index = 0 }) {
  ...
  const tier = (product.tiers || [])[0] || { label: product.period || 'Produk', price: product.price, priceIntl: product.priceIntl }
  // Badge durasi ringkas dari durasi utama: "3 Bulan" → "3bln"/"3m", "1 Bulan" → "1bln"/"1m".
  const durBadge = durationBadge(tier.label, t)
  // Tier promo sintetis dengan harga flash (kedua mata uang)
  const selTier = {
    label: `${tier.label} (Flash Sale)`,
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Nama produk AI + badge durasi (1bln/3bln/1thn) durasi utama — pengganti
