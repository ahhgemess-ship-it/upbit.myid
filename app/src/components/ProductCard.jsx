...
export default function ProductCard({ product: rawProduct, index = 0 }) {
  ...
  // Pilihan durasi lengkap ada di halaman detail produk (dropdown di sana).
  const tiers = product.tiers || []
  const tier = tiers[0] || { label: product.period || 'Produk', price: product.price, priceIntl: product.priceIntl }
  // Badge durasi ringkas durasi utama: "3 Bulan" → "3bln"/"3m". Null untuk tier non-durasi.
  ...
  // Durasi utama dengan harga setelah diskon (kedua mata uang), supaya keranjang konsisten.
  const chosenTier = () => ({
    ...effectiveTier,
          ...
          <div className="pc-price-row">
            ...
            {percent > 0 && (
              <span className="pc-disc">
                <span className="pc-strike" style={{ fontSize: 12.5 }}>{fmt(amountOf(tier))}</span>
